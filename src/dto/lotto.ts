...
export const LottoTypeDef = objectType({
    name: 'Lotto',
    description:
...
const shuffle: <T>(list: T[], count: number) => T[] = (list, count) => {
    const result = [...list];
...
export const makeLotto: () => Lotto = () => {
    const nums = Array.from({length: 45}, (_, i) => i + 1);
    const shuffled = shuffle(nums, 7);
    const additional = shuffled.pop()!;
    const numbers = shuffled.sort((a, b) => a - b);
